...
FROM VECINO V
INNER JOIN MIEMBRO M ON M.ID_VECINO = V.ID
INNER JOIN COMITE C ON C.ID = M.ID_COMITE
WHERE C.ID = 1
...
--Ejercicio 3
SELECT ESTADO_DENUNCIA.ESTADO, 
COUNT(DENUNCIA.ID_ESTADO) 
FROM ESTADO_DENUNCIA 
INNER JOIN DENUNCIA 
ON ESTADO_DENUNCIA.ID = DENUNCIA.ID_ESTADO 
WHERE ESTADO_DENUNCIA.ID = DENUNCIA.ID_ESTADO 
GROUP BY DENUNCIA.ID_ESTADO, ESTADO_DENUNCIA.ESTADO
